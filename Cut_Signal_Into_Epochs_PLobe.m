function [Epochs, SleepStages] = Cut_Signal_Into_Epochs_PLobe(edf_files,hypno_files)
% Ryan Byrne
% Function for cutting the epoch into signals using the Parietal Lobe
...
for k = 1:numFiles
    ...
    parietal_EEG = data.EEGPz_Oz ; 

    %Number of samples in frontal EEG channel
    num_samples = numel(parietal_EEG) * numel(parietal_EEG{1});
    ...
    time_series_EEG = [] ; 
    for j = 1:numel(parietal_EEG)
        time_series_EEG = vertcat(time_series_EEG, parietal_EEG{j}) ; 
    end
